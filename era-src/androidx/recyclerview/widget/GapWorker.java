...
final class GapWorker implements Runnable {
  ...
  private void buildTaskList() {
    ...
    int j;
    for (j = 0; b1 < i; j = k) {
      RecyclerView recyclerView = this.mRecyclerViews.get(b1);
      int k = j;
    ...
    b1 = 0;
    while (b2 < i) {
      RecyclerView recyclerView = this.mRecyclerViews.get(b2);
      if (recyclerView.getWindowVisibility() == 0) {
        LayoutPrefetchRegistryImpl layoutPrefetchRegistryImpl = recyclerView.mPrefetchRegistry;
        int k = Math.abs(layoutPrefetchRegistryImpl.mPrefetchDx) + Math.abs(layoutPrefetchRegistryImpl.mPrefetchDy);
        for (j = 0; j < layoutPrefetchRegistryImpl.mCount * 2; j += 2) {
          Task task;
          boolean bool;
          if (b1 >= this.mTasks.size()) {
            task = new Task();
            this.mTasks.add(task);
          } else {
            task = this.mTasks.get(b1);
          } 
          int m = layoutPrefetchRegistryImpl.mPrefetchArray[j + 1];
          if (m <= k) {
            bool = true;
          } else {
            bool = false;
          ...
          task.position = layoutPrefetchRegistryImpl.mPrefetchArray[j];
          b1++;
        } 
      } 
      b2++;
...
/* Location:              /home/platinum/Documents/AndroidRE/com.guanxu.technology.snaptain_era_s5c_29_apps.evozi.com-dex2jar.jar!/androidx/recyclerview/widget/GapWorker.class
