...
public class DFSPacketSplitter {
  ...
  public DFSPacketSplitter(Callback paramCallback) {
  ...
  private boolean isPackageStart(byte paramByte) {
    int i = this.state;
    if ((i == 0 || i == 1) && paramByte == -91) {
      this.state = 1;
    } else if (this.state == 1 && paramByte == 42) {
      this.state = 2;
    } else {
      if (this.state == 2 && paramByte == 29) {
        this.suspectPackageSize = 42;
        return true;
      } 
      if (this.state == 1 && paramByte == 30) {
        this.state = 3;
      } else {
        if (this.state == 3 && paramByte == 34) {
          this.suspectPackageSize = 30;
          return true;
        } 
        if (this.state == 1 && paramByte == 7) {
          this.state = 4;
        } else {
          if (this.state == 4 && paramByte == 40) {
            this.suspectPackageSize = 7;
            return true;
          } 
          if (this.state == 1 && paramByte == 46) {
            this.state = 5;
          } else {
            if (this.state == 5 && paramByte == 9) {
              this.suspectPackageSize = 46;
              return true;
            } 
            this.state = 0;
          } 
  ...
  private boolean verifyPackage(byte[] paramArrayOfbyte) {
    ...
    if (i < 2)
      return false; 
    if (getCheckSum(paramArrayOfbyte, 2, paramArrayOfbyte.length - 3) == paramArrayOfbyte[paramArrayOfbyte.length - 2])
      bool = true; 
    return bool;
  }
...
/* Location:              /home/platinum/Documents/AndroidRE/com.guanxu.technology.snaptain_era_s5c_29_apps.evozi.com-dex2jar.jar!/com/netopsun/rxtxprotocol/dfs_gps_protocol/DFSPacketSplitter.class
