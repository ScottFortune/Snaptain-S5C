...
public class ViewDragHelper {
  ...
  private void ensureMotionHistorySizeForId(int paramInt) {
    ...
    if (arrayOfFloat == null || arrayOfFloat.length <= paramInt) {
      ...
      this.mLastMotionY = arrayOfFloat;
        ...
        if (i != 2) {
          ...
          for (m = k; m < j; m++) {
            ...
            if (isValidPointerForActionMove(k)) {
              ...
              float f4 = f1 - this.mInitialMotionX[k];
              float f2 = f3 - this.mInitialMotionY[k];
              reportNewEdgeDrags(f4, f2, k);
              if (this.mDragState == 1)
                break; 
              View view = findTopChildUnder((int)f1, (int)f3);
              if (checkTouchSlop(view, f4, f2) && tryCaptureViewForDrag(view, k))
                break; 
            } 
          } 
          saveLastMotion(paramMotionEvent);
...
/* Location:              /home/platinum/Documents/AndroidRE/com.guanxu.technology.snaptain_era_s5c_29_apps.evozi.com-dex2jar.jar!/androidx/customview/widget/ViewDragHelper.class
